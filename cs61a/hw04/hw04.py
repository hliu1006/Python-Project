...
def balanced(m):
    ...
    if is_weight(m):
        ...
        a = length(left(m)) * total_weight(left_end)
        b = length(right(m)) * total_weight(right_end)
        return a == b and balanced(left_end) and balanced(right_end)
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, type_password):
        nonlocal balance, count, attemps
        if type_password == password and count < 3:
            if amount > balance:
            ...
            count += 1
            if len(attemps) == 3:
                str_list = 'Your account is locked. Attempts: '
                return str_list + str(attemps)
            if type_password != password:
                attemps.append(type_password)
